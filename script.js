  ...
  if (data.length > 0) {
    data.forEach((medicine) => {
      ...
      p.addEventListener("click", async () => {
        ...
        document.querySelector(".medicine-section-list").style.display =
          "block";
      });
...
const fetchMedicinesByCategory = async (categoryId) => {
  try {
    const response = await fetch(
      `https://cliniqueplushealthcare.com.ng/prescriptions/get_drug_class_by_id/${categoryId}`
    );
...
const remarkInput = document
  .querySelector(".save-pricription-form")
